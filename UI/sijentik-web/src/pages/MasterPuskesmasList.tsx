  ...
  return (
    <div className="flex-1 overflow-hidden p-6 bg-background-light flex flex-col gap-4">
      <PageHeader
        ...
        breadcrumbs={[{ label: "Data Master" }, { label: "Puskesmas" }]}
        actions={
      ...
      {user?.role !== "ADMIN" && (
        <SearchBar placeholder="Cari ID, Puskesmas, Kepala Puskesmas dll..." />
      )}
          ...
          <table className="min-w-full divide-y divide-border-subtle text-left">
            <thead className="bg-slate-50 sticky top-0 z-10 shadow-sm">
              <tr>
                ...
                <th className="px-4 py-3 text-xs font-semibold text-text-muted uppercase tracking-wider border-b border-border-subtle w-24 text-center">
                  Aksi
                </th>
              </tr>
            </thead>
    ...
    <tr className={`hover:bg-primary/5 transition-colors h-10 ${bg}`}>
      ...
      <td className="px-4 py-2 whitespace-nowrap text-center">
        <div className="flex items-center justify-center gap-1">
          <Link
            to={`/master/puskesmas/detail/${fullId}`}
          ...
          <Link
            to={`/master/puskesmas/edit/${fullId}`}
            className="inline-flex items-center justify-center w-8 h-8 rounded text-text-muted hover:text-primary hover:bg-primary/5 transition-colors"
            title="Edit"
          >
